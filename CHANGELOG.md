...
- **Runtime dependency: `marfrit-openvino +p18` (patches 0003–0042).**
  ...
  the tables held only the resident pairs, reading one row before the
  activation buffer. Bisected on the served binary and measured fixed
  (the 35B with the CPU tier serves on the B60 at ratio 99, 23.6 t/s).
