...
# GUARD (added 2026-09-20 after review): the defaults are the SAME as
# build-openvino.sh, so the natural invocation would cmake --install on top of
# the measurement plugin's prefix. This script exists only to build a
# debug-caps VARIANT, so refuse to run unless the caller names a distinct
# prefix. OV_SRC is deliberately not guarded -- it may share the source clone
# -- but note that BUILD=$SRC/build-prod is shared then, so the debug-caps
...
log "Configuring (Release, debug caps ON, plugin set matching the wheel)"
cmake -S . -B "$BUILD" -G Ninja \
  ...
  -DENABLE_DEBUG_CAPS=ON -DENABLE_GPU_DEBUG_CAPS=ON \
