...
| `text_config.num_experts_per_tok` | absent (dense) / 8 | 10 | new key, not read (no code path needs the active-expert count; the host/device slot formulas size all resident experts, not the per-token active subset) **[CORRECTED 2026-09-15, CODE pass: this is the single most load-bearing number in the FreeToken design and marking it 'not read' is where arcint stopped being FreeToken-shaped. 10 of 512 = 1.95 % active per token; FreeToken reads it (`models/qwen4_exp/config.py:232`) and its whole offload mechanism is built on it. The clause is true of the FIT ARITHMETIC and false as a design conclusion: computing all 512 does 51.2x the expert work the architecture requires. See `docs/research-freetoken-code.md` §3.]** | — |
